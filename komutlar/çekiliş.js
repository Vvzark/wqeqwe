...
let prefix = ayarlar.prefix
   if(!message.member.hasPermission("ADMINISTRATOR")) return message.channel.send(new Discord.MessageEmbed().setColor("RED").setDescription(`Bu Komutu Kullanabilmek İçin **\`YÖNETİCİ\`** Yetkisine Sahip Olman Gerek.`))
  ...
  if(args[0] == "başlat"){   
    ...
    const yanlış = new Discord.MessageEmbed()
     .setColor("#00ffdc")
     .setDescription("> **Çekiliş Ne Kadar Sürecek? (`x saniye`, `x dakika`, `x saat`, `x gün` Şeklinde Yazınız)**")
                      ...
                      await sleep(50)
                        let kazananherkes = db.fetch(`cekilis_${msg.id}.kazananlar`)
                           ...
                           msg.channel.send(`<@${kazananherkes.join('>, <@')}>`)
                            msg.channel.send(new Discord.MessageEmbed().setColor("#00ffdc").setDescription(`**Tebrikler** <@${kazananherkes.join('>, <@')}>! **\`${verilecek}\` Çekilişini Kazandınız!**`))
   ...
   const yanlış = new Discord.MessageEmbed()
     .setColor("#00ffdc")
     .setDescription(`Lütfen **\`${prefix}çekiliş başlat\`** Şeklinde Kullanınız!`)
