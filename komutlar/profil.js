...
exports.run = async (bot, msg, args) => {
        ...
        userinfo.id = user.id;
        userinfo.od1 = msg.guild.members.cache.get(user.id).user.presence.game || "Oynadığı Bir Oyun Yok." || "Şuanlık Bir Oyun Oynamıyor."
        ...
        .addField(`Sahip Olduğu Roller :`, `${msg.guild.members.cache.get(user.id).roles.cache.filter(r => r.name !== "@everyone").map(r => r).join(' **|** ') || "**Bu Kullanıcıda Hiç Rol Bulunmuyor!**"}`, false)
...
exports.conf = {
  enabled: true,
  guildOnly: true,
  aliases: ['kullanıcı-bilgim','kullanıcı-bilgi','kullanıcıbilgi','hesap','hesapbilgi','hesap-bilgi','profil'],
  permLevel: 0
};
exports.help = {
  name: 'kullanıcıbilgim',
  description: 'Hesap Bilgilerin Verir.',
  usage: 'hesap'
};